...
const SETTINGS_CACHE_KEY = 'bitblog_settings_cache_v1';
  ...
  try {
    ...
    if (cached) {
      ...
      return {
        ...defaultSettings,
        ...parsed,
        posts_per_page: Number(parsed.posts_per_page) || defaultSettings.posts_per_page,
        recommended_posts_count: Number(parsed.recommended_posts_count) || defaultSettings.recommended_posts_count,
...
export const SettingsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    try {
      ...
      if (res && res.data) {
        const merged: SiteSettings = {
          ...
          recommended_posts_count: Number(res.data.recommended_posts_count) || 4,
        };
        setSettings(merged);
        try {
  ...
  const updateSettings = async (newSettings: Partial<SiteSettings>) => {
    const updatedState: SiteSettings = {
      ...settings,
      ...newSettings,
      comments_enabled: newSettings.comments_enabled !== undefined ? Boolean(newSettings.comments_enabled) : settings.comments_enabled,
      newsletter_enabled: newSettings.newsletter_enabled !== undefined ? Boolean(newSettings.newsletter_enabled) : settings.newsletter_enabled,
      posts_per_page: newSettings.posts_per_page !== undefined ? (Number(newSettings.posts_per_page) || 10) : settings.posts_per_page,
      recommended_posts_count: newSettings.recommended_posts_count !== undefined ? (Number(newSettings.recommended_posts_count) || 4) : (settings.recommended_posts_count || 4),
    };

    setSettings(updatedState);
    try {
      localStorage.setItem(SETTINGS_CACHE_KEY, JSON.stringify(updatedState));
    } catch {}

    const res = await ApiService.updateSettings(newSettings);
    if (res && res.data) {
      const serverMerged: SiteSettings = {
        ...defaultSettings,
        ...res.data,
        comments_enabled: res.data.comments_enabled === true || res.data.comments_enabled === 'true',
        newsletter_enabled: res.data.newsletter_enabled === true || res.data.newsletter_enabled === 'true',
      ...
      try {
        localStorage.setItem(SETTINGS_CACHE_KEY, JSON.stringify(serverMerged));
      } catch {}
    }
    return res;
  };
