...
export const Profile: React.FC = () => {
  ...
  const [rawImageForEditor, setRawImageForEditor] = useState<string>('');
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > maxSizeBytes) {
      setErrorMsg(`File size (${(file.size / (1024 * 1024)).toFixed(1)} MB) exceeds limit.`);
      return;
    }

    const objectUrl = URL.createObjectURL(file);
    setRawImageForEditor(objectUrl);
    setPhotoEditorOpen(true);
    e.target.value = '';
  };

  const handlePhotoEditorApply = (editedFile: File, previewUrl: string) => {
    setImageFile(editedFile);
    setImagePreview(previewUrl);
    setSuccessMsg('Photo refined & framed! Click "Save Changes" or "Save Chosen Image Now" to upload.');
