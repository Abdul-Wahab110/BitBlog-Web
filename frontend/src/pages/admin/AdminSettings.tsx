          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '1.25rem' }}>
            <div>
              ...
              <input
                ...
                onChange={e => handleChange('posts_per_page', e.target.value === '' ? '' : Math.max(1, Math.min(50, parseInt(e.target.value) || 1)))}
                style={{ width: '100%', padding: '0.55rem 0.75rem', fontSize: '0.85rem' }}
              />
              <small style={{ color: 'var(--color-muted)', fontSize: '0.75rem', display: 'block', marginTop: '0.3rem' }}>
                Controls how many latest articles are displayed per page across Home and Blog feeds (1 - 50).
              </small>
            </div>

            <div>
              <label htmlFor="recommended-posts-count" style={{ display: 'block', fontWeight: 600, marginBottom: '0.25rem', fontSize: '0.83rem' }}>Recommended Articles Count</label>
              <input
                id="recommended-posts-count"
                type="number"
                min={1}
                max={20}
                value={settings.recommended_posts_count ?? 4}
                onChange={e => handleChange('recommended_posts_count', e.target.value === '' ? '' : Math.max(1, Math.min(20, parseInt(e.target.value) || 1)))}
                style={{ width: '100%', padding: '0.55rem 0.75rem', fontSize: '0.85rem' }}
              />
              <small style={{ color: 'var(--color-muted)', fontSize: '0.75rem', display: 'block', marginTop: '0.3rem' }}>
                Controls how many top curated stories appear in the Recommended section on the Home page (1 - 20).
              </small>
            </div>
