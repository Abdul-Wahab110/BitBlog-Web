...
import { AvatarPhotoEditorModal } from '../../components/common/AvatarPhotoEditorModal';
...
export const AdminUsers: React.FC = () => {
  ...
  const [adminPhotoEditorOpen, setAdminPhotoEditorOpen] = useState(false);
  const [adminPhotoEditorSrc, setAdminPhotoEditorSrc] = useState<string>('');
  ...
  const handleEditFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setAdminPhotoEditorSrc(objectUrl);
    setAdminPhotoEditorOpen(true);
    e.target.value = '';
  };
  ...
  // Deduplicate users by user_id to prevent any duplicate entries
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h1 style={{ fontSize: '1.6rem', marginBottom: '0.2rem', fontFamily: 'var(--font-heading)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <Users size={24} color="var(--color-secondary)" /> User Management & RBAC Roles
          </h1>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexShrink: 0 }}>
          <span style={{ fontSize: '0.82rem', color: 'var(--color-text-secondary)', fontWeight: 600 }}>Filter Role:</span>
          <select
            value={roleFilter}
            onChange={e => setRoleFilter(e.target.value)}
            style={{ padding: '0.45rem 0.75rem', fontSize: '0.85rem', fontWeight: 600, borderRadius: 'var(--radius-md)' }}
          >
            <option value="all">All Roles ({uniqueUsers.length})</option>
      ...
      {loading ? (
        <LoadingState message="Fetching system user records from database..." />
      ) : filteredUsers.length === 0 ? (
        <EmptyState title="No Users Found" description="No user records match your query." />
      ) : isMobile ? (
        /* Mobile Cards View - ONLY rendered on mobile (< 768px) */
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
          {filteredUsers.map(user => {
            const targetIsSystemAdmin = isSystemAdmin(user);
            const isOwner = currentUser?.user_id === user.user_id;
            ...
            return (
              ...
              >
                {/* 1. Header: Avatar + Name + Username + Status */}
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '0.75rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', minWidth: 0 }}>
                    ...
                    <div style={{ minWidth: 0 }}>
                      <div style={{ fontWeight: 700, fontSize: '0.95rem', color: 'var(--color-text)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                        {user.name}
                      </div>
                  ...
                  {isAdmin && !targetIsSystemAdmin ? (
                    ...
                    >
                      <span style={{ width: '6px', height: '6px', borderRadius: '50%', backgroundColor: user.status === 'ACTIVE' ? 'var(--color-success)' : 'var(--color-danger)' }} />
                      {user.status}
                    </button>
                  ) : (
                    <span
                ...
                {/* 2. Metadata Grid: Email & Role & Stories */}
                <div
                  style={{
                    display: 'grid',
                ...
                >
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.15rem' }}>
                    <span style={{ fontSize: '0.7rem', fontWeight: 600, color: 'var(--color-muted)', textTransform: 'uppercase', letterSpacing: '0.03em' }}>
                      Email
                    </span>
                    <a
                    ...
                    >
                      <Mail size={12} color="var(--color-secondary)" />
                      {user.email}
                    </a>
                  </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.15rem' }}>
                    ...
                    <span style={{ fontWeight: 700, fontSize: '0.82rem', display: 'flex', alignItems: 'center', gap: '0.3rem', color: 'var(--color-text)' }}>
                      <FileText size={12} color="var(--color-secondary)" />
                      {user.post_count || 0} Stories
                    </span>
                  </div>
                </div>

                {/* 3. Action Toolbar */}
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '0.6rem', borderTop: '1px solid var(--color-border)', paddingTop: '0.75rem' }}>
                  <button
                    type="button"
                  ...
                  >
                    <Edit size={14} /> Edit Profile & Role
                  </button>

                  {targetIsSystemAdmin ? (
                    <span
                      title="Protected Website Owner & System Administrator"
                      style={{
                    ...
                    >
                      <Lock size={13} color="var(--color-secondary)" /> Protected
                    </span>
                  ) : isAdmin && !isOwner ? (
                    <button
                      type="button"
        ...
        /* Desktop Table View - ONLY rendered on desktop (>= 768px) */
        <div className="table-responsive" style={{ backgroundColor: 'var(--color-card)', borderRadius: 'var(--radius-lg)', border: '1px solid var(--color-border)' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.875rem' }}>
            <thead>
              <tr style={{ backgroundColor: 'var(--color-surface-alt)', borderBottom: '1px solid var(--color-border)', color: 'var(--color-text-secondary)', fontWeight: 700 }}>
                ...
                <th style={{ padding: '0.85rem 1rem' }}>Articles</th>
                  ...
                  <tr key={user.user_id} style={{ borderBottom: '1px solid var(--color-border)' }}>
                    <td style={{ padding: '0.75rem 1rem', fontWeight: 600 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.65rem' }}>
                        ...
                        />
                        <div>
                          <span style={{ color: 'var(--color-text)', display: 'block' }}>{user.name}</span>
                          <span style={{ fontSize: '0.75rem', color: 'var(--color-muted)' }}>@{user.username}</span>
                        </div>
                      </div>
                    </td>
                    ...
                    <td style={{ padding: '0.75rem 1rem' }}>
                      <span style={{ fontWeight: 600, color: 'var(--color-text)' }}>{user.post_count || 0}</span>
                    </td>
                    <td style={{ padding: '0.75rem 1rem' }}>
                      {isAdmin && !targetIsSystemAdmin ? (
                        <button
      ...
      {/* Professional Profile Photo Editor Studio Modal */}
      <AvatarPhotoEditorModal
        isOpen={adminPhotoEditorOpen}
        imageSrc={adminPhotoEditorSrc}
        onClose={() => setAdminPhotoEditorOpen(false)}
