...
export interface SiteSettings {
  ...
  recommended_posts_count?: number;
...
export class SettingModel {
    ...
    try {
      ...
      if (rows && rows.length > 0) {
        for (const row of rows) {
          const rawKey = row.SETTING_KEY || row.setting_key;
          const val = row.SETTING_VALUE || row.setting_value;
          if (rawKey) {
            const key = rawKey.toLowerCase();
            if (val === 'true') dbSettings[key] = true;
            else if (val === 'false') dbSettings[key] = false;
            else if (val !== null && val !== undefined && !isNaN(Number(val)) && String(val).trim() !== '' && !String(val).startsWith('http') && !String(val).includes('@')) {
      ...
      for (const [k, v] of Object.entries(rawStoreSettings)) {
        ...
        if (v === 'true') parsedStoreSettings[lowerK] = true;
        else if (v === 'false') parsedStoreSettings[lowerK] = false;
        else if (lowerK === 'posts_per_page') parsedStoreSettings[lowerK] = Number(v) || 10;
        else if (lowerK === 'recommended_posts_count') parsedStoreSettings[lowerK] = Number(v) || 4;
        else parsedStoreSettings[lowerK] = v;
      }
      ...
      if (merged.recommended_posts_count !== undefined) merged.recommended_posts_count = Number(merged.recommended_posts_count) || 4;
      ...
      return {
        ...defaultSettings,
        ...parsed,
        posts_per_page: Number(parsed.posts_per_page) || 10,
        recommended_posts_count: Number(parsed.recommended_posts_count) || 4,
        comments_enabled: parsed.comments_enabled === true || parsed.comments_enabled === 'true',
        newsletter_enabled: parsed.newsletter_enabled === true || parsed.newsletter_enabled === 'true',
      };
    }
  }
  ...
  public static async updateSettings(data: Partial<SiteSettings>): Promise<SiteSettings> {
    ...
    for (const [key, value] of Object.entries(data)) {
      if (value !== undefined) {
        store.settings[key.toLowerCase()] = String(value);
      }
    }
    Database.saveStore();
    ...
    try {
      for (const [key, value] of Object.entries(data)) {
        if (value !== undefined) {
          ...
          const checkSql = `SELECT setting_id FROM site_settings WHERE LOWER(setting_key) = :1`;
          const existing = await Database.execute<{ SETTING_ID: number }>(checkSql, [lowerKey]);

          if (existing && existing.length > 0) {
            const updateSql = `UPDATE site_settings SET setting_value = :1, updated_at = CURRENT_TIMESTAMP WHERE LOWER(setting_key) = :2`;
            await Database.execute(updateSql, [strValue, lowerKey]);
          } else {
            const insertSql = `INSERT INTO site_settings (setting_key, setting_value) VALUES (:1, :2)`;
            await Database.execute(insertSql, [lowerKey, strValue]);
          }
        }
      }
    } catch (err) {
