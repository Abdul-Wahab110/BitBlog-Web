...
export class PostController {
  public static async getPosts(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const defaultLimit = Number(settings.posts_per_page) || 10;
      const limit = req.query.limit ? (parseInt(req.query.limit as string) || defaultLimit) : defaultLimit;
      ...
      ResponseUtil.success(res, result.posts, 'Published articles retrieved', 200, result.pagination);
    } catch (error) {
      next(error);
    }
  }
